...
function createRenderer(options) {
    ...
    function mountChildren(children, container, parentComponent) {
        children.forEach(function (v) { return patch(null, v, container, parentComponent, null); });
    }
...
function getSequence(arr) {
    var len = arr.length;
    var min_arr = [0]; // 存储最小的索引，以索引0为基准
    var prev_arr = arr.slice(); // 储存前面的索引，slice为浅复制一个新的数组
    var last_index;
    var start;
    var end;
    var middle;
    for (var i_5 = 0; i_5 < len; i_5++) {
        ...
        if (arrI !== 0) {
            ...
            if (arr[min_arr[end]] > arrI) {
                min_arr[end] = i_5;
                if (end > 0) {
                    prev_arr[i_5] = min_arr[end - 1]; // 前面的索引
                }
            }
        }
    }
    // 从最后一项往前查找
    var result = [];
    var i = min_arr.length;
    var last = min_arr[i - 1];
    while (i-- > 0) {
        result[i] = last;
        last = prev_arr[last];
    }
...
export { createApp, createRenderer, createTextVNode, effect, getCurrentInstance, h, inject, provide, ref, renderSlots };
